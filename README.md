...
+ Job ID: jobs 명령어로 확인할 수 있는 현재 셸에서 관리되는 **백그라운드 작업 번호**이다. %1, %2 와 같이 사용한다.
...
| 목적 | 명령어 구문 | 설명 |
| :--- | :--- | :--- |
| **정상적인 종료 요청** | `kill 1234` 또는 `kill -TERM 1234` | PID가 1234인 프로세스에게 부드러운 종료(15번, TERM)를 요청한다. |
| **강제 종료** | `kill -9 1234` 또는 `kill -KILL 1234` | PID가 1234인 프로세스를 즉시, 강제로 종료한다. (가장 강력 !) |
| **설정 재로딩** | `kill -HUP 5678` | PID가 5678인 웹 서버 등에게 설정 파일을 다시 읽으라고 알린다. |
| **백그라운드 작업 종료** | `kill %1` 또는 `kill -9 %1` | `jobs` 목록에서 작업 번호가 1인 백그라운드 작업을 종료한다. |

### kill 명령어 사용 예시 1

#### ps 명령어로 프로세스 확인

먼저, ps 명령어로 **프로세스를 확인하고 PID**를 얻을 수 있다.
...
위 예제에서 python3 프로세스의 PID는 5678이다.
...
이 명령어는 5678 PID를 가진 python3 프로세스에게 **SIGTERM(15) 시그널을 보내 정상 종료를 요청**한다. 

이 시그널은 프로세스가 정상 종료하도록 유도하지만, 프로세스가 이를 무시하거나 응답하지 않을 수 있다.
