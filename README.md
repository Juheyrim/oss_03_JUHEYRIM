...
| 옵션                    | 설명                                        | 예시                    |                         |
| --------------------- | ----------------------------------------- | --------------------- | ----------------------- |
| **ps -e**               | 시스템에서 실행 중인 모든 프로세스를 출력           | `ps -e`               |                         |
| **ps -f**               | 풀 포맷 으로 자세한 정보 출력        | `ps -f`               |                         |
| **ps -ef**              | 전체 프로세스를 상세 정보와 함께 출력     | `ps -ef`              |                         |
| **ps aux**              | CPU·메모리 정보를 포함해 다양한 항목 표시 | `ps aux`              |                         |
| **ps aux**               | grep <name>`                              | 특정 프로세스만 검색할 때 사용 | `ps aux \| grep python` |
| **ps aux --sort=-%cpu** | CPU 사용량 기준으로 정렬해 높은 순으로 출력                | `ps aux --sort=-%cpu` |                         |
| **ps -u <user>**        | 특정 사용자가 실행한 프로세스만 확인                      | `ps -u root`          |                         |
